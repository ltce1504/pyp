...
class Student:
    def __init__(self):
        # Create three empty buckets at once
        self.rno, self.nm, self.p = [], [], []

    def add(self):
        # Grab and throw data straight into the lists
        self.rno.append(input("Enter Roll: "))
        self.nm.append(input("Enter Name: "))

    def add_marks(self):
        # Just append marks
        self.p.append(input("Enter Marks: "))

    def display(self):
        print("\nRoll  Name  Marks")
        # One loop to rule them all
        for i in range(len(self.rno)):
            print(f"{self.rno[i]}     {self.nm[i]}     {self.p[i]}")
